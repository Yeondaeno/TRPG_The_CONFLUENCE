...
> **구현 시점의 결정 (A + C)** — ADR-003은 "프록시 없이 브라우저에서 직접
> 호출"을 전제했는데, `web/index.html`을 더블클릭해 여는 `file://` 페이지는
> 출처가 **`null`** 입니다. 공급자가 CORS를 열어놨더라도 `Origin: null`은
> 대개 거부되므로, **클라우드 공급자가 실제로 붙는지는 확인되지 않았습니다**
> (샌드박스에서 재봤지만 브라우저가 외부에 아예 못 나가 대조군까지 실패해
> 판정 불가였습니다 — 그래서 전부 `corsOk: null`(미확인)입니다).
>
> 그래서 두 갈래를 함께 갑니다.
>
> - **A. 그대로 만들고 실패를 정직하게 알린다.** 클라우드 공급자는 선택·시도
>   가능하되, 막히면 "브라우저에서 직접 호출할 수 없습니다"와 함께 원인
>   후보(`file://` 출처 / CORS / 키 / 네트워크)를 그대로 보여줍니다.
> - **C. 로컬 모델을 1급으로 둔다.** Ollama·LM Studio는 사용자가 자기
>   컴퓨터에서 CORS를 열 수 있는 **유일하게 확실한 경로**라, `custom`에
>   숨기지 않고 별도 공급자로 앞에 세웁니다.
>
> 어느 쪽도 안 되면 게임은 그대로 굴러갑니다 — 그게 이 명세의 제1원칙입니다.
