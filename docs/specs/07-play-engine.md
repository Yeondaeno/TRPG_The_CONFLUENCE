...
## 추가(후속): 누가 굴리는가 — 캐릭터 점유

명세 07 초판은 판정 버튼을 방 안 **누구나** 누를 수 있게 두었습니다. 혼자
플레이할 때는 맞지만, 여럿이 붙으면 아무나 남의 캐릭터를 대신 굴릴 수 있어
이상합니다. `ROOM.claims`(캐릭터 점유)는 이미 있었는데 판정 버튼이 그걸
보지 않았습니다.

`Game.rollPermission(room, playerName, charName)`이 규칙을 갖습니다.

| 상황 | 누가 굴리나 |
|---|---|
| 점유자가 있는 캐릭터 | **그 사람만** (GM도 대신 못 굴린다) |
| 아무도 아무것도 점유 안 함(혼자 플레이) | 제한 없음 |
| 점유가 시작된 방의 주인 없는 캐릭터 | GM |
| 위에서 GM도 없으면 | 아무나 (진행이 멈추는 것보다 낫다) |

**막는 게 아니라 "누가 굴리는가"입니다.** 시도자를 내 캐릭터로 바꾸면 같은
선택지도 다시 눌립니다. 잠긴 이유는 화면에 그대로 씁니다 — 남의 캐릭터면
점유자를 지목하고, 주인이 없으면 GM 이름을 말합니다. 아무 일도 안 일어나는
것처럼 보이면 안 됩니다.

적용 범위는 **주사위를 굴리는 모든 곳**입니다: 씬 선택지 판정, 자유 행동
판정, 전투(공격·사망 판정·안정화·임의 판정·차례 넘기기). 적 차례는 주인이
없으므로 누구나 진행시킬 수 있습니다.

판정이 아닌 선택(이동, "주변을 살핀다" 같은 판정 없는 선택지)은 그대로
누구나 누를 수 있습니다 — 파티의 합의 사항이지 개인의 굴림이 아닙니다.
